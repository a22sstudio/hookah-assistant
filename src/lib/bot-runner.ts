...
import { processMasterMessage, transcribeAudio } from '@/lib/ai'
...
function createBot(): Telegraf {
  ...
  bot.command('help', async (ctx) => {
    ...
    return ctx.reply(
      `📋 Команды\n\n` +
        ...
        `  "хочу новые шланги"\n` +
        `  "чего мало?"\n` +
        `  "поставь Марата на завтра"\n` +
        (isSenior ? `  "зарплата Марата за сентябрь"\n` : '') +
        `\n🎤 Голосовое — распознаю и обработаю.\n` +
        `📸 Фото накладной — распознаю и оформлю приход.\n` +
        `📄 PDF накладной — извлеку текст и оформлю приход.\n` +
        (isSenior
  ...
  // Фото → рекомендуем использовать веб-панель «Поставки»
  bot.on('photo', async (ctx) => {
    ...
    return ctx.reply(
      '📦 Распознавание накладных по фото отключено — теперь для этого есть удобный модуль «Поставки» в веб-панели.\n\n' +
      'Открой вкладку «Поставки» → «Новая поставка» → «ИИ из текста» — вставь текст из накладной (Ctrl+V) и нажми «Распознать».\n\n' +
      'Это быстрее и точнее, чем распознавание по фото.',
    )
  })

  // Документы — рекомендуем загрузить через веб-панель
  bot.on('document', async (ctx) => {
    ...
    const doc = (ctx.message as { document?: { file_name?: string } }).document
    const fileName = doc?.file_name || 'файл'

    return ctx.reply(
      `📄 Файл «${fileName}» получен.\n\n` +
      'Распознавание PDF/накладных переведено в веб-панель. Открой вкладку «Поставки» → «Новая поставка» → «ИИ из текста», скопируй текст из PDF и вставь его (Ctrl+V) в поле — система автоматически распознает позиции.',
    )
  })
